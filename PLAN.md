...
- 读取资源和待审核投稿时合并“转变卡 / 种族 → 传承”、“碎心者工具集 / 碎心者 → 工具书”、“单人游玩 → 扩展规则”，去重并移除其余非标准内容标签。保存后写入标准值，审核历史读取保留原始记录。
...
- 公开资源影响明细：`docs/content-tag-changes.md`。可用 `node scripts/report_content_tag_changes.mjs bootstrap.json report.md` 重新生成。
